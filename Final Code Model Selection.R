...
# AIC
# AIC is not given in regsubsets output,
# but BIC - n.par*log(n) + 2*n.par
# get number of parameters in each model to calculate AIC:
n.par <- apply(summ$which,1, sum)
# get length of Y after excluding any cases with missing values
# note: here there are no missing values, so easy, just take length of Y
sum(is.na(statedata))
n <- length(statedata[,1])
aic <- bic - n.par*log(n) + 2*n.par
# which models are within +2 of model with smallest AIC?
cbind(aic, selected)[(aic < (min(aic) +2)), ]

# Note, if there would be missing values, this would work
n <- sum(apply(1-is.na(statedata),1,prod) )
# because 
#apply(1-is.na(statedata),1,prod)
# gives a 0 for a case with a missing value in Y or any of its predictors
# and 1 otherwise
